function GraphPlotNodesDegreeDistribution(Graph,varargin)
...
try 
    ...
    if ~isempty(strmatch(lower(DataType),{'in','both'},'exact'))
        ...
        plot(XScale_in,Y_in,'db','MarkerSize',8,'LineWidth',3);
        ...
        if ~isempty(InFitRange)
            ...
            set(hErrorBar_in,'LineWidth',2);
            DynamicLegend{end+1}=['Incoming links fit: Slope='  num2str(p(1))];
        end
    end
    if ~isempty(strmatch(lower(DataType),{'out','both'},'exact'))
        ...
        plot(XScale_out,Y_out,'dg','MarkerSize',8,'LineWidth',3);  
